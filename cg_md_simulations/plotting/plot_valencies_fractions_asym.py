...
df_asym = pd.read_csv('../results/diamseries_asym_gd0.8.csv')
f, ax = plt.subplots(figsize = (8, 5))
df_sym = df_sa_8.copy()
df_sym = df_sym[df_sym['diel']==15]
df_asym['meanfrac'] = (df_asym['frac1']+df_asym['frac2'])/2

df_sym_indiv = df_sym[['diam', 'frac']]
df_sym_indiv['typ'] = 'sym'
df_asym_indiv = df_asym[['diam', 'frac1', 'frac2', 'meanfrac']]
df_asym_indiv['frac'] = df_asym_indiv['meanfrac']
df_asym_indiv['typ'] = 'asym'
df_indiv = pd.concat((df_sym_indiv, df_asym_indiv), axis = 0)
...
asym_ct = 0
for typ in ['sym', 'asym']:
    for diam in [111, 168, 255]:
        if typ == 'asym' and diam == 111:
            diam = 126
        df_hue = df_indiv[np.logical_and(df_indiv['typ']==typ, df_indiv['diam']==diam)]
        
        for frac in df_hue['frac']:
            plt.scatter(ct+0.1, frac*100, c = colors[ct], alpha = 0.7, s = 15)
            
        if typ == 'asym':
            for frac in df_hue['frac1']:
                plt.scatter(ct+0.2, frac*100, c = col2[asym_ct], alpha = 0.7, s = 15)
            for frac in df_hue['frac2']:
                plt.scatter(ct+0.2, frac*100, c = col2[asym_ct], alpha = 0.7, s = 15)
            asym_ct+=1
        ct+=1
